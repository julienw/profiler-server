...
export function profileRoutes() {
   ...
   */
  router.delete('/profile/:profileToken', jwt({
    ...
    if (!ctx.params.profileToken) {
      throw new BadRequestError(`Expected to get a profile token in the path of the form "/delete-profile/PROFILE_TOKEN"`);
    }

    // Verify there is a valid JWT token.
    let profileToken: string;
    const jwtData: unknown = ctx.state.jwtData;
    if (!jwtData || typeof jwtData !== 'object' || typeof jwtData.profileToken !== 'string') {
      if (ctx.state.jwtOriginalError) {
        const error = ctx.state.jwtOriginalError;
        // The JWT decoding middleware found an error earlier.
        log.warn('jwt_invalid', `An error was thrown while trying to decode the JWT token: (${error.name}) ${error.message}`);
        throw new ForbiddenError('A valid authentication token is needed to execute this operation.');
      }
    ...
    if (profileToken !== ctx.params.profileToken) {
      const message = 'The profileToken in the JWT did not match the token provided in the path.';
      log.warn('jwt_profileTokenMismatch', message);
      throw new ForbiddenError(message);
    }

    log.debug('attempting', 'Attempt to delete the profile from Google Cloud Storage.');
    ...
    try {
      ...
      if ('code' in error && 'message' in error) {
        ...
        if (code === 404) {
          throw new NotFoundError('That profile was most likely already deleted.');
        }
        throw error;
      }
      console.log(error);
