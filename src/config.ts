...
function validateConfigValues(config: Config) {
  ...
  if (googleAuthenticationFilePath && googleAuthenticationFilePath !== 'MOCKED') {
    ...
    try {
      ...
      log.critical('gcs_configuration_error', `The authentication file '${googleAuthenticationFilePath}' is missing or not readable.`);
      throw new ConfigurationError(`The authentication file is missing or not readable.`);
    }
  }
}
...
type Config = {
  ...
  readonly googleAuthenticationFilePath: string;
  readonly jwtSecret: string;
  readonly bitlyToken: string;
};

export const config: Config = loadConfig();
log.info('configuration_success', 'Configuration info has been loaded successfully.');
