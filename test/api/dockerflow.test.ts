...
describe('dockerflow endpoints', () => {
  ...
  function setupForHeartbeat({
    failBitly
  }: $Shape<{readonly failBitly: boolean;}> = {}) {
  ...
  describe('__heartbeat__', () => {
    ...
    it('answers an error to the heartbeat if the bucket does not exist', async () => {
      jest.spyOn(Bucket.prototype, 'exists').mockReturnValue(Promise.resolve([false]));
      jest.spyOn(process.stdout, 'write').mockImplementation(() => true);
      const {
        agent
      } = setupForHeartbeat();
      await agent.get('/__heartbeat__').expect(500);
      expect(process.stdout.write).toHaveBeenCalledWith(expect.stringContaining('server_error'));
    });
    ...
    it('answers an error to the heartbeat if bitly is down', async () => {
      ...
      const {
        agent,
        nockScope
      } = setupForHeartbeat({ failBitly: true });
  ...
  describe('__version__', () => {
    ...
    it('answers to the version endpoint when the file is present', async () => {
      ...
      jest.spyOn(fs.promises, 'stat').mockResolvedValue({ mtime: fakeLastModifiedDate });
      const agent = setup();
      const response = await agent.get('/__version__').expect('Content-Type', /^application\/json/).expect('Last-Modified', fakeLastModifiedDate.toUTCString()).expect(200);
